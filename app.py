...
@app.route('/start_container_form')
def start_container_form():
    return render_template('start_container_form.html')
...
def start_container():
    ...
    if container_id:
        try:
            ...
            flash(f'Failed to start container: {str(e)}', 'danger')
    else:
        flash('Container ID is required.', 'danger')

    return redirect(url_for('start_container_form'))
